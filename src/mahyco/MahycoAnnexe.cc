 ...
 */
/*---------------------------------------------------------------------------*/
CartesianInterface::ICartesianMesh*
Mahyco::MahycoModule::
_initCartMesh() {
...
void
Mahyco::MahycoModule::
hydroStartInitEnvAndMat(MahycoHydroStartInitEnvAndMatVars& vars)
{
  PROF_ACC_BEGIN(__FUNCTION__);
...
void
Mahyco::MahycoModule::
initEnvForAcc(MahycoInitEnvForAccVars& vars) {
  PROF_ACC_BEGIN(__FUNCTION__);
 ...
 *******************************************************************************
 */
void Mahyco::MahycoModule::prepareFaceGroupForBc(MahycoPrepareFaceGroupForBcVars& vars) {
  PROF_ACC_BEGIN(__FUNCTION__);
  ...
  ENUMERATE_FACE(iface, allFaces()){
     ...
     for (NodeEnumerator inode(face.nodes()); inode.index() < face.nbNode(); ++inode) {
         if (vars.m_node_coord[inode].x > options()->threshold)  flag_x0 = false;
         if (vars.m_node_coord[inode].y > options()->threshold)  flag_y0 = false;
         if (vars.m_node_coord[inode].z > options()->threshold)  flag_z0 = false;
         if (math::abs(vars.m_node_coord[inode].x - maxCoor.x) > options()->threshold) flag_xmax = false;
         if (math::abs(vars.m_node_coord[inode].y - maxCoor.y) > options()->threshold) flag_ymax = false;
         if (math::abs(vars.m_node_coord[inode].z - maxCoor.z) > options()->threshold) flag_zmax = false;
     }
...
/*---------------------------------------------------------------------------*/
void Mahyco::MahycoModule::
initBoundaryConditionsForAcc(MahycoInitBoundaryConditionsForAccVars& vars) {
  PROF_ACC_BEGIN(__FUNCTION__);
  ...
  // de manière concurrente aux données.
  {
    ...
    for (Integer i = 0, nb = options()->boundaryCondition.size(); i < nb; ++i){
      ...
      Types_mahyco::BoundaryConditionType type = options()->boundaryCondition[i]->type();
