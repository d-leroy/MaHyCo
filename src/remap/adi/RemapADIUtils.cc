...
computeAndLimitGradPhi(RemapADIComputeAndLimitGradPhiVars& vars,
        int projectionLimiterId, Face frontFace, Face backFace, 
   ...
   if (projectionLimiterId < minmodG) {
    ...
    for (Integer ivar = 0; ivar < nb_vars; ivar++) {
      vars.m_grad_phi[cell][ivar] =
          fluxLimiterG(projectionLimiterId, vars.m_grad_phi_face[frontFace][ivar], 
 ...
 */
 ...
 fluxLimiter(RemapADIFluxLimiterVars& vars,
        int projectionLimiterId, Real r) {
        ...
        const Integer nb_vars)
{
  ...
  for (Integer ivar = 0; ivar < nb_vars; ivar++) {
    ...
    xg = computexgxd(vars.m_phi_lagrange[cell][ivar], 
                    vars.m_phi_lagrange[frontcell][ivar], 
                    vars.m_phi_lagrange[backcell][ivar], 
                    h0, y0plus, y0moins, 0);
    xd = computexgxd(vars.m_phi_lagrange[cell][ivar], 
                    vars.m_phi_lagrange[frontcell][ivar], 
                    vars.m_phi_lagrange[backcell][ivar], 
                    h0, y0plus, y0moins, 1);
    // calcul des valeurs sur ces points d'intersections
    yg = computeygyd(vars.m_phi_lagrange[cell][ivar], 
                    vars.m_phi_lagrange[frontcell][ivar], 
                    vars.m_phi_lagrange[backcell][ivar], 
                    h0, y0plus, y0moins, vars.m_grad_phi[cell][ivar], 0);
    yd = computeygyd(vars.m_phi_lagrange[cell][ivar], 
                    vars.m_phi_lagrange[frontcell][ivar], 
                    vars.m_phi_lagrange[backcell][ivar], 
        ...
        // -h0/2. et -h0/2.+abs(face_normal_velocity)*deltat_n
    {
      ...
      flux1m = INTY(-h0 / 2. + partie_positive_v, -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      // Flux1m : integrale -inf,  -h0/2.
      flux1 = INTY(-h0 / 2., -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      ...
      flux3m = INTY(-h0 / 2. + partie_positive_v, xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar]);
      // Flux3 : integrale -inf,  -h0/2.
      flux3 = INTY(-h0 / 2., xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar]);
      // integrale positive
      Flux[ivar] = std::max(
      ...
      flux1m = INTY(partie_positive_dual_v, -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      // Flux1m : integrale -inf,  0..
      flux1 = INTY(0., -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      ...
      flux3m = INTY(partie_positive_dual_v, xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar]);
      // Flux3 : integrale -inf, 0.
      flux3 = INTY(0., xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar]);
      // integrale positive
      ...
      //
        ...
        flux1 = INTY(h0 / 2. - partie_positive_v, -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
        // Flux1m : integrale -inf,  -h0/2.
        flux1m = INTY(h0 / 2., -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
        //
        ...
        flux3 = INTY(h0 / 2. - partie_positive_v, xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar] );
        // Flux3m : integrale -inf,  -h0/2.
        flux3m = INTY(h0 / 2., xd, yd, h0 / 2., vars.m_phi_lagrange[frontcell][ivar] );
        //
        // integrale positive
        Flux[ivar] = std::max(
            ((flux1m - flux1) + (flux2m - flux2) + (flux3m - flux3)), 0.);
        ...
        int nb_vars)
{
  ...
  for (Integer ivar = 0; ivar < nb_vars; ivar++) {
                    ...
                    // -h0/2. et -h0/2.+abs(face_normal_velocity)*deltat_n
    {
      // Flux1m : integrale -inf,  -h0/2.+partie_positive_v
      flux1m =
          INTY(-h0 / 2. + partie_positive_v, -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      // Flux1m : integrale -inf,  -h0/2.
      flux1 = INTY(-h0 / 2., -h0 / 2., vars.m_phi_lagrange[backcell][ivar], xg, yg);
      //
      // Flux2m : integrale -inf,  -h0/2.+partie_positive_v
