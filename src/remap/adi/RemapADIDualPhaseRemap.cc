...
computeDualUremap(RemapADIComputeDualUremapVars& vars, const Integer idir, const Integer nb_env)
{
  ...
  if (options()->ordreProjection > 1) {
    ...
    if (options()->projectionLimiteurId < Types_mahyco::Limiteur::MinmodG) {
      
      // Il faut spécialiser pour les limiteurs < minmodG
      // Spécialisation
      switch (options()->projectionLimiteurId) {
        case Types_mahyco::Limiteur::Minmod  : computeDualGradPhi_LimC<MinMod>   (idir); break;
        case Types_mahyco::Limiteur::SuperBee: computeDualGradPhi_LimC<SuperBee> (idir); break;
        case Types_mahyco::Limiteur::VanLeer : computeDualGradPhi_LimC<VanLeer>  (idir); break;
        default      : computeDualGradPhi_LimC<DefaultO1>(idir);
      }
    }
    else { // (options()->projectionLimiteurId > minmodG) PAS SUR GPU
      vars.m_dual_grad_phi.fill(0.0);
      ...
      ENUMERATE_NODE(inode, ndm.innerNodes()) {
        ...
        RemapADIServiceBase<RemapADIService>::computeDualGradPhi(node, frontfrontnode, frontnode, backnode, backbacknode, idir);   
      }
    }
#if 0
    vars.m_dual_grad_phi.synchronize();
#else
    auto queue_synchronize = m_acc_env->refQueueAsync();
    m_acc_env->vsyncMng()->globalSynchronize(queue_synchronize, vars.m_dual_grad_phi);
#endif
  }
  ...
  ENUMERATE_NODE(inode, allNodes()){
    for (Integer index_env=0; index_env < nb_env; index_env++) {
      vars.m_back_flux_mass_env[inode][index_env] =0.;
      vars.m_front_flux_mass_env[inode][index_env] = 0.;
    }
    vars.m_back_flux_mass[inode] = 0.;
    vars.m_front_flux_mass[inode] = 0.;
  }
  ...
  ENUMERATE_FACE(iface, fdm.allFaces()) {
    ...
    if (cellb.localId() != -1) {
     ...
     Real3 outer_face_normalb(vars.m_outer_face_normal[cellb][indexfacecellb]);
//      Real outer_face_normal_dirb = math::dot(outer_face_normalb, dirproj);
     ...
     ENUMERATE_NODE(inode, face.nodes()) {
        ...
        vars.m_back_flux_mass_env[inode][index_env] += 
        oneovernbcell * vars.m_dual_phi_flux[cellb][nb_env+index_env];
        // pour le flux total
        vars.m_back_flux_mass[inode]  +=  oneovernbcell * vars.m_dual_phi_flux[cellb][nb_env+index_env];
        }
     }
    }
    ...
    if (cellf.localId() != -1) {
      ...
      Real3 outer_face_normalf(vars.m_outer_face_normal[cellf][indexfacecellf]);
//       Real outer_face_normal_dirf = math::dot(outer_face_normalf, dirproj);
      ...
      ENUMERATE_NODE(inode, face.nodes()) {
        ...
        vars.m_front_flux_mass_env[inode][index_env] += 
        oneovernbcell * vars.m_dual_phi_flux[cellf][nb_env+index_env];
        // pour le flux total
        vars.m_front_flux_mass[inode] += oneovernbcell * vars.m_dual_phi_flux[cellf][nb_env+index_env];
        }
      }
    }
  }
  ...
  auto cfc = m_acc_env->connectivityView().cellFace();
  {
    ...
    auto in_dual_phi_flux      = ax::viewIn(command_f, vars.m_dual_phi_flux    );
    
    auto out_back_flux_contrib_env   = ax::viewOut(command_f, vars.m_back_flux_contrib_env );
    auto out_front_flux_contrib_env  = ax::viewOut(command_f, vars.m_front_flux_contrib_env );
  ...
  if (mesh()->dimension() == 2)
  {
    ...
    auto in_back_flux_contrib_env   = ax::viewIn(command, vars.m_back_flux_contrib_env );
    auto in_front_flux_contrib_env  = ax::viewIn(command, vars.m_front_flux_contrib_env );
 
    auto inout_back_flux_mass      = ax::viewInOut(command,vars.m_back_flux_mass     );
    auto inout_front_flux_mass     = ax::viewInOut(command,vars.m_front_flux_mass    );
    auto inout_back_flux_mass_env  = ax::viewInOut(command,vars.m_back_flux_mass_env );
    auto inout_front_flux_mass_env = ax::viewInOut(command,vars.m_front_flux_mass_env);
...
#if 0
  vars.m_back_flux_mass_env.synchronize(); 
  vars.m_front_flux_mass_env.synchronize();
  vars.m_back_flux_mass.synchronize(); 
  vars.m_front_flux_mass.synchronize();
#else
  MeshVariableSynchronizerList mvsl(m_acc_env->vsyncMng());

  mvsl.add(vars.m_back_flux_mass_env);
  mvsl.add(vars.m_front_flux_mass_env);
  mvsl.add(vars.m_back_flux_mass);
  mvsl.add(vars.m_front_flux_mass);
...
#if 1
  {
    ...
    auto in_back_flux_mass  = ax::viewIn(command, vars.m_back_flux_mass );
    auto in_front_flux_mass = ax::viewIn(command, vars.m_front_flux_mass);
    auto in_dual_grad_phi   = ax::viewIn(command, vars.m_dual_grad_phi  );
    auto in_node_coord      = ax::viewIn(command, vars.m_node_coord     );
    
    auto inout_u_dual_lagrange   = ax::viewInOut(command, vars.m_u_dual_lagrange  );
    auto inout_phi_dual_lagrange = ax::viewInOut(command, vars.m_phi_dual_lagrange);
  ...
  
  {
    ...
    Real thresold = options()->getArithmeticThreshold();
  ...
  ENUMERATE_NODE(inode, ndm.innerNodes()) {
    ...
    // flux de masse
    vars.m_u_dual_lagrange[inode][3] += vars.m_back_flux_mass[inode] - vars.m_front_flux_mass[inode];
    ...
    // vitesse = vitesse(pNode) si FrontFluxMasse(pNode) > 0 et vitesse(voisin devant) sinon 
    Integer signfront;
    Real ufront = 0.5 * (vars.m_phi_dual_lagrange[node][idir] + vars.m_phi_dual_lagrange[frontnode][idir]);

    if (ufront > 0)  signfront = 1;
    else signfront = -1;
    if (vars.m_front_flux_mass[inode] < 0.) {
      // vittese front upwind
      FrontupwindVelocity[0] = vars.m_phi_dual_lagrange[frontnode][0]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[frontnode][0]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      FrontupwindVelocity[1] = vars.m_phi_dual_lagrange[frontnode][1]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[frontnode][1]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      FrontupwindVelocity[2] = vars.m_phi_dual_lagrange[frontnode][2]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[frontnode][2]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      // energie cinetique
      FrontupwindEcin = vars.m_phi_dual_lagrange[frontnode][4]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[frontnode][4] * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
    
    } else {
      // vittese front upwind
      FrontupwindVelocity[0] = vars.m_phi_dual_lagrange[node][0]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][0]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      FrontupwindVelocity[1] = vars.m_phi_dual_lagrange[node][1]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][1]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      FrontupwindVelocity[2] = vars.m_phi_dual_lagrange[node][2]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][2]  * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
      
      // energie cinetique
      FrontupwindEcin = vars.m_phi_dual_lagrange[node][4]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][4] * 
      (signfront * (vars.m_node_coord[frontnode][idir] - vars.m_node_coord[node][idir]) - deltat * ufront));
    ...
    // Backvitesse = vitesse(voisin amont)) si backFluxMasse(pNode) > 0 et vitesse(pNode) sinon
    Integer signback;
    Real  uback= 0.5 * (vars.m_phi_dual_lagrange[backnode][idir] + vars.m_phi_dual_lagrange[node][idir]);

    if (uback > 0) signback = 1;
    else signback= -1;

    if (vars.m_back_flux_mass[inode] > 0.) {
      // vittese back upwind
      BackupwindVelocity[0] = vars.m_phi_dual_lagrange[backnode][0]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[backnode][0]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      BackupwindVelocity[1] = vars.m_phi_dual_lagrange[backnode][1]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[backnode][1]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      BackupwindVelocity[2] = vars.m_phi_dual_lagrange[backnode][2]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[backnode][2]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      // energie cinetique
      BackupwindEcin = vars.m_phi_dual_lagrange[backnode][4]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[backnode][4] * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      
    } else {
      // vittese back upwind
      BackupwindVelocity[0] = vars.m_phi_dual_lagrange[node][0]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][0]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      BackupwindVelocity[1] = vars.m_phi_dual_lagrange[node][1]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][1]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      BackupwindVelocity[2] = vars.m_phi_dual_lagrange[node][2]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][2]  * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
      // energie cinetique
      BackupwindEcin = vars.m_phi_dual_lagrange[node][4]
      + order2 * ( 0.5 * vars.m_dual_grad_phi[node][4] * 
      (signback * (vars.m_node_coord[node][idir] - vars.m_node_coord[backnode][idir]) - deltat * uback));
      
    }
     
    vars.m_u_dual_lagrange[inode][0] += vars.m_back_flux_mass[inode] * BackupwindVelocity[0] - 
        vars.m_front_flux_mass[inode] * FrontupwindVelocity[0];
    vars.m_u_dual_lagrange[inode][1] += vars.m_back_flux_mass[inode] * BackupwindVelocity[1] - 
        vars.m_front_flux_mass[inode] * FrontupwindVelocity[1];
    vars.m_u_dual_lagrange[inode][2] += vars.m_back_flux_mass[inode] * BackupwindVelocity[2] - 
        vars.m_front_flux_mass[inode] * FrontupwindVelocity[2];
    // energie cinetique
    vars.m_u_dual_lagrange[inode][4] += vars.m_back_flux_mass[inode] * BackupwindEcin - 
        vars.m_front_flux_mass[inode] * FrontupwindEcin;
  }
  ...
  ENUMERATE_NODE(inode, ndm.innerNodes()) {
    ...
    // filtre des valeurs abherentes
    if (abs(vars.m_u_dual_lagrange[node][0]) < vars.m_arithmetic_thresold) vars.m_u_dual_lagrange[node][0]=0.;
    if (abs(vars.m_u_dual_lagrange[node][1]) < vars.m_arithmetic_thresold) vars.m_u_dual_lagrange[node][1]=0.;
    if (abs(vars.m_u_dual_lagrange[node][2]) < vars.m_arithmetic_thresold) vars.m_u_dual_lagrange[node][2]=0.;
    // recalcul des phi apres cette projection
    // phi vitesse
    vars.m_phi_dual_lagrange[node][0] = vars.m_u_dual_lagrange[inode][0] /  vars.m_u_dual_lagrange[inode][3];
    vars.m_phi_dual_lagrange[node][1] = vars.m_u_dual_lagrange[inode][1] /  vars.m_u_dual_lagrange[inode][3];
    vars.m_phi_dual_lagrange[node][2] = vars.m_u_dual_lagrange[inode][2] /  vars.m_u_dual_lagrange[inode][3];
    // Phi masse nodale
    vars.m_phi_dual_lagrange[node][3] = vars.m_u_dual_lagrange[inode][3] ;
    // Phi energie
    vars.m_phi_dual_lagrange[node][4] = vars.m_u_dual_lagrange[inode][4] /  vars.m_u_dual_lagrange[inode][3];
  }
#endif
}
